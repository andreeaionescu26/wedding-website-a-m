...
import { submitRSVP, updateRSVPMessage } from '../utils/rsvpService';
...
const RSVPForm = ({ setView }) => {
  ...
  const handleSubmit = async () => {
    ...
    const rsvpData = {
      ...
      declineMessage: null // Will be added later if provided
    };

    const result = await submitRSVP(rsvpData);

    if (result.success) {
      // Store the RSVP ID for potential message update
      setSubmittedRsvpId(result.id);
      
      // Check if any guests are attending
      const attending = guests.some(g => g.attending === 'yes');
      setHasAttendingGuests(attending);
  ...
  const handleMessageSubmit = async (message) => {
    if (submittedRsvpId && message) {
      await updateRSVPMessage(submittedRsvpId, message);
    }
  };
